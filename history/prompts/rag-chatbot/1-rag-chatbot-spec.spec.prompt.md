...
# /sp.specify — Integrated RAG Chatbot for Physical AI Book

You are operating in **Spec-Kit Plus /sp.specify mode**.

Define specifications for an **Integrated RAG Chatbot** embedded inside the
...
Build a **Retrieval-Augmented Generation (RAG) chatbot** that can:

- Answer questions about the full book content
- Answer questions based **only on user-selected text**
- Work as an **embedded assistant inside the book UI**
...
- Reasoning Model: **Claude (via OpenRouter)**
- Embeddings Model: **Qwen Embeddings (via OpenRouter)**
- Backend API: **FastAPI**
- Vector DB: **Qdrant Cloud (Free Tier)**
- Metadata DB: **Neon Serverless PostgreSQL**
- Frontend Embed: **Docusaurus (React-based widget)**
...
1. **Global Book Q&A**
   - Retrieve answers from all indexed chapters

2. **Selection-Based Q&A**
   - User selects text → chatbot answers using **only that context**

3. **Citation-Aware Responses**
   - Responses reference chapter/module/section

4. **Strict RAG Guardrails**
   - No hallucination outside retrieved context
   - If context not found → say "Not covered in this book"

---

## DATA PIPELINE

- Input: Markdown chapters (Docusaurus source)
- Chunking: Section-aware (heading-based)
- Embeddings: Qwen (OpenRouter)
- Storage:
  - Vectors → Qdrant
...
- No general ChatGPT-style assistant
- No non-book knowledge answers
...
- Selection-based queries never leak global context
- Works within free-tier limits
- Easily deployable with the book
...
- ✅ Impact: Created comprehensive specification for RAG chatbot feature that enables book Q&A functionality
