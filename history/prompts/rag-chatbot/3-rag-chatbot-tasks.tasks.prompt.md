...
# /sp.task — RAG Chatbot (Short Version)

You are operating in **/sp.task mode (Spec-Kit Plus)**.

Goal: Add a **RAG chatbot** to the Docusaurus book that answers questions from book content and user-selected text.
...
- **FastAPI backend** (separate `/backend` folder)
- **OpenRouter API** (LLM routing)
- **Claude + Qwen** (embeddings)
- **Qdrant Cloud (Free Tier)** – vectors
...
- Add `.env` & `.env.example`
- Install FastAPI, qdrant-client, psycopg, httpx

### 2️⃣ Ingestion & Embeddings
- Read Docusaurus markdown
- Chunk (500–1000 tokens)
- Generate embeddings (Claude/Qwen via OpenRouter)
...
- Top-K vector search
- Context-only prompt (no hallucinations)
...
### 5️⃣ Frontend (Docusaurus)
- Floating chat widget
- "Ask about selected text" option
- Show sources in response

### 6️⃣ Deployment
- Backend → Vercel Functions
- Frontend → GitHub → Vercel
- Keys via Vercel ENV (not in code)
...
## ✅ DONE WHEN
- Chatbot answers only from book
...
▶ Next step:
**/sp.implement — RAG Chatbot Integration**
...
**Success Criteria**:
- [ ] Successfully reads all markdown files from specified directory
...
- [ ] Chunk size stays within 500-1000 token range
- [ ] Section context is preserved in chunks
- [ ] No information is lost during chunking
...
**Success Criteria**:
- [ ] Successfully generates embeddings via OpenRouter
- [ ] Handles API errors and rate limits properly
- [ ] Embeddings are stored in correct format
...
**Success Criteria**:
- [ ] Top-K search returns relevant results
...
**Success Criteria**:
- [ ] Global queries use full book context appropriately
- [ ] Selection queries use only selected text context
- [ ] No information leakage between contexts
- [ ] Prompts are properly formatted for Claude
...
**Success Criteria**:
- [ ] Successfully generates responses via Claude
...
**Success Criteria**:
- [ ] Endpoint responds with status information
- [ ] System checks work properly
...
- [ ] Documents are properly processed and indexed
...
- [ ] Both global and selection queries supported
...
- 🧪 Tests: All tasks include testable success criteria and dependencies
- 📁 Files: Created tasks.md with detailed implementation tasks and quickstart.md
- 🔁 Next prompts: Ready for /sp.implement to execute the tasks
- 🧠 Reflection: Task breakdown addresses all required functionality with proper sequencing
